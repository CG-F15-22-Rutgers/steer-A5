	...
	if (!_neighbors.empty()) {
		for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbour = _neighbors.begin(); neighbour != _neighbors.end(); neighbour++)
		{
			...
			else
			{
				...
				away_obs = away_obs + away_obs_tmp * (_SocialForcesParams.sf_wall_a * exp((((this->radius()) - (this->position() - min_stuff.second).length()) / _SocialForcesParams.sf_agent_b)));
			}
		}
	}
	...
	if (!_neighbors.empty()) {
		...
		for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbour = _neighbors.begin(); neighbour != _neighbors.end(); neighbour++)
		{
			...
			if (tmp_ob->computePenetration(this->position(), this->radius()) > 0.000001)
			{
				...
				wall_repulsion_force = wall_repulsion_force + wall_normal * (radius() - min_stuff.first) * _SocialForcesParams.sf_body_force;
